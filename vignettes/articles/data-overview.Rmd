...
```{r database scheme, echo=FALSE, fig.cap="Figure 1: `mpathsenser` database scheme. The underlined fields constitute the primary key of the table.", out.width = '100%'}
knitr::include_graphics("https://github.com/koenniem/mpathsenser/blob/master/inst/extdata/mpathsenser_db.png?raw=true")
```
...
Note that primary keys are [underlined]{.underline} in the tables below.
...
tribble(
  ...
  mutate(Variable = cell_spec(Variable, underline = Variable == "study_id")) |>
...
tribble(
  ...
  mutate(Variable = cell_spec(Variable, underline = Variable == "participant_id")) |>
  kbl(booktabs = TRUE, escape = FALSE) |>
  table_style()
```
...
tribble(
  ...
  mutate(Variable = cell_spec(Variable, underline = Variable == "file_name")) |>
  kbl(booktabs = TRUE, escape = FALSE) |>
  table_style()
```
...
tribble(
  ...
  mutate(
    Variable = cell_spec(Variable, underline = Variable %in% c("participant_id", "date", "time"))
  ) |>
  kbl(booktabs = TRUE, escape = FALSE) |>
  table_style() |>
  column_spec(1, color = c(rep("auto", 4), rep("red", 3), rep("auto", 41)))
```
...
tribble(
  ...
  mutate(
    Variable = cell_spec(
      Variable,
      underline = Variable %in% c("participant_id", "date", "time", "app")
    )
  ) |>
  kbl(booktabs = TRUE, escape = FALSE) |>
  table_style()
...
tribble(
  ...
  mutate(
    Variable = cell_spec(
      Variable,
      underline = Variable %in% c("participant_id", "date", "time", "bluetooth_device_id")
    )
  ) |>
  kbl(booktabs = TRUE, escape = FALSE) |>
  table_style()
...
### BluetoothBeacon
Resulting from a specific Bluetooth scan. See the relevant [m-Path manual page](https://m-path.io/manual/knowledge-base/bluetooth-triggers-beacon/).

```{r bluetoothbeacon-table, echo=FALSE}
tribble(
  ~Variable, ~Type, ~Description,
  "measurement_id", "character", "Uniquely generated measurement ID. Currently not present in the data.",
  "participant_id", "character", "Unique ID of the participant.",
  "date", "character", "The date of the measurement. Represented as a character as YYYY-MM-DD.",
  "time", "character", "The start time of the measurement. Represented in 24-hour time as 00:00:00.000000.",
  "region", "character", "The name of the region (beacon region) in which the beacon was detected.",
  "uuid", "character", "The universally unique identifier (UUID) of the beacon.",
  "rssi", "integer", "The Received Signal Strength Indicator (RSSI) signal strength to the beacon in decibels referenced to one milliwatt (dBm) ranging from 0 to -100.",
  "major", "integer", "The major value of the beacon, used to group beacons within a region.",
  "minor", "integer", "The minor value of the beacon, used to identify individual beacons within a group.",
  "accuracy", "double", "The accuracy of the proximity measurement in metres. A negative value indicates that the accuracy could not be determined.",
  "proximity", "character", "An estimate of the proximity of the beacon:
  <ul>
    <li> immediate: The beacon is very close.
    <li> near: The beacon is relatively close.
    <li> far: The beacon is far away.
    <li> unknown: The proximity could not be determined.
  </ul>"
) |>
  mutate(
    Variable = cell_spec(
      Variable,
      underline = Variable %in% c("participant_id", "date", "time", "uuid")
    )
  ) |>
  kbl(booktabs = TRUE, escape = FALSE) |>
  table_style()
...
tribble(
  ...
  mutate(
    Variable = cell_spec(
      Variable,
      underline = Variable %in%
        c("participant_id", "date", "time", "event_id", "calendar_id", "start", "end")
    )
  ) |>
  kbl(booktabs = TRUE, escape = FALSE) |>
  table_style()
...
The `Heartbeat` sensor is not so much a sensor, but rather a signal that is sent to the m-Path server to let it know that it is still active. If such a signal was not received after a certain period of time (configurable), a notification is sent to the phone that data collection has stopped and that the app should be reopened. Note that even if a heartbeat is present, it does not guarantee that data can actually be collected.
...
The following tables describe data collected from Garmin wearable devices as they are represented in the mpathsenser. These sensors provide high-resolution physiological and movement data. See also the [m-Path manual](https://manual.m-path.io/knowledge-base/wearables-garmin-direct-integration/#garmin-measurement-types) for more information on the raw data.

### GarminAccelerometer

```{r garminaccelerometer-table, echo=FALSE}
tribble(
  ~Variable        , ~Type       , ~Description                                                                         ,
  "measurement_id" , "character" , "Uniquely generated measurement ID. Currently not present in the data."              ,
  "participant_id" , "character" , "Unique ID of the participant."                                                      ,
...
```{r garminactigraphy-table, echo=FALSE}
tribble(
  ~Variable              , ~Type       , ~Description                                                                                              ,
  "measurement_id"       , "character" , "Uniquely generated measurement ID. Currently not present in the data."                                   ,
  "participant_id"       , "character" , "Unique ID of the participant."                                                                           ,
  ...
  mutate(
    Variable = cell_spec(
      Variable,
      underline = Variable %in% c("participant_id", "date", "time", "instance")
    )
  ) |>
  kbl(booktabs = TRUE, escape = FALSE) |>
  table_style()
...
```{r garminenhancedbbi-table, echo=FALSE}
tribble(
  ~Variable        , ~Type       , ~Description                                                                                      ,
  "measurement_id" , "character" , "Uniquely generated measurement ID. Currently not present in the data."                           ,
  "participant_id" , "character" , "Unique ID of the participant."                                                                   ,
...
```{r garmingyroscope-table, echo=FALSE}
tribble(
  ~Variable        , ~Type       , ~Description                                                                         ,
  "measurement_id" , "character" , "Uniquely generated measurement ID. Currently not present in the data."              ,
  "participant_id" , "character" , "Unique ID of the participant."                                                      ,
...
```{r garminheartrate-table, echo=FALSE}
tribble(
  ~Variable        , ~Type       , ~Description                                                                         ,
  "measurement_id" , "character" , "Uniquely generated measurement ID. Currently not present in the data."              ,
  "participant_id" , "character" , "Unique ID of the participant."                                                      ,
...
```{r garminrespiration-table, echo=FALSE}
tribble(
  ~Variable        , ~Type       , ~Description                                                                           ,
  "measurement_id" , "character" , "Uniquely generated measurement ID. Currently not present in the data."                ,
  "participant_id" , "character" , "Unique ID of the participant."                                                        ,
...
```{r garminskintemperature-table, echo=FALSE}
tribble(
  ~Variable        , ~Type       , ~Description                                                                         ,
  "measurement_id" , "character" , "Uniquely generated measurement ID. Currently not present in the data."              ,
  "participant_id" , "character" , "Unique ID of the participant."                                                      ,
...
```{r garminsteps-table, echo=FALSE}
tribble(
  ~Variable        , ~Type       , ~Description                                                                         ,
  "measurement_id" , "character" , "Uniquely generated measurement ID. Currently not present in the data."              ,
  "participant_id" , "character" , "Unique ID of the participant."                                                      ,
...
```{r garminstress-table, echo=FALSE}
tribble(
  ~Variable        , ~Type       , ~Description                                                                         ,
  "measurement_id" , "character" , "Uniquely generated measurement ID. Currently not present in the data."              ,
  "participant_id" , "character" , "Unique ID of the participant."                                                      ,
...
```{r garminwriststatus-table, echo=FALSE}
tribble(
  ~Variable        , ~Type       , ~Description                                                                                       ,
  "measurement_id" , "character" , "Uniquely generated measurement ID. Currently not present in the data."                            ,
  "participant_id" , "character" , "Unique ID of the participant."                                                                    ,
...
```{r garminzerocrossing-table, echo=FALSE}
tribble(
  ~Variable         , ~Type       , ~Description                                                                                              ,
  "measurement_id"  , "character" , "Uniquely generated measurement ID. Currently not present in the data."                                   ,
  "participant_id"  , "character" , "Unique ID of the participant."                                                                           ,
