...
which_sensor <- function(data, sensor) {
	switch(sensor,
				 ...
				 apps = CARP:::apps_fun(data),
				 battery = CARP:::battery_fun(data),
				 bluetooth = CARP:::bluetooth_fun(data),
				 calendar = CARP:::calendar_fun(data),
				 connectivity = CARP:::connectivity_fun(data),
				 device = CARP:::device_fun(data),
				 gyroscope = CARP:::gyroscope_fun(data),
				 light = CARP:::light_fun(data),
				 location = CARP:::location_fun(data),
				 memory = CARP:::memory_fun(data),
				 mobility = CARP:::mobility_fun(data),
				 screen = CARP:::screen_fun(data),
				 text_message = CARP:::text_message_fun(data),
				 weather = CARP:::weather_fun(data),
				 wifi = CARP:::wifi_fun(data))
}
...
apps_fun <- function(data) {
	data$body <- lapply(data$body, function(x) x$body)
	data$body <- lapply(data$body, function(x) tibble::tibble(timestamp = x$timestamp, apps = list(x$installed_apps)))
	data <- tidyr::unnest(data, body)
	# data$timestamp <- as.POSIXct(data$timestamp, "%Y-%m-%dT%H:%M:%S", tz="Europe/Brussels")
	data
}
