...
The `coverage()` function computes how many samples per hour were collected for each sensor, either in absolute numbers or relative to the expected sampling rate (see `coverage_frequency()`). Set `metric = "time"` with `expected` to measure the fraction of time covered instead of the number of samples. Only the first and last bins of a participant's span are prorated, so a participant starting at 13:50 can still reach full coverage for the 13:00--14:00 bin. The resulting coverage chart is a quick way to spot participants or sensors with poor data collection.
